...
function renderNetWorth() {
  ...
  headerNetWorth.textContent = formatMoney(netWorth.current, { sign: netWorth.current < 0 ? "-" : "" });
  headerNetWorthChange.textContent = `${formatMoney(netWorth.change, { sign: changeSign })} from start`;
...
function renderFinancialHud() {
  ...
  hudUpcomingOutflow.textContent = formatMoney(upcomingOutflow, { sign: upcomingOutflow > 0 ? "-" : "" });
  hudCreditUsage.textContent = formatMoney(creditUsage, { sign: creditUsage > 0 ? "-" : "" });
...
function renderSelectedDayStrip() {
  ...
  selectedStripExpense.textContent = formatMoney(totals.outflow, { sign: totals.outflow > 0 ? "-" : "" });
...
function renderSummaryView() {
  ...
  summaryNetWorth.textContent = formatMoney(netWorth.current, { sign: netWorth.current < 0 ? "-" : "" });
  summaryNetWorthChange.textContent = `${formatMoney(netWorth.change, { sign: changeSign })} from start`;
  summaryNetWorthChange.className = netWorth.change >= 0 ? "positive-text" : "negative-text";
  startingNetWorthDisplay.textContent = formatMoney(netWorth.start, { sign: netWorth.start < 0 ? "-" : "" });
...
function renderDonutChart(chartElement, legendElement, segments) {
  ...
  legendElement.innerHTML = positiveSegments.map(segment => {
    ...
    .forEach(item => {
      ...
      li.innerHTML = `
        ...
        <span class="transaction-copy">
          ...
          <small>${escapeHtml(new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric" }).format(new Date(item.createdAt)))}</small>
        </span>
...
function exportCsv() {
  ...
  getAllTransactions()
    .slice()
    .sort((a, b) => a.dateKey.localeCompare(b.dateKey) || String(a.createdAt).localeCompare(String(b.createdAt)))
    .forEach(item => {
      rows.push([
        item.dateKey,
...
function importBackupFile(file) {
  ...
  reader.onload = () => {
    try {
      ...
      const confirmed = window.confirm("Restore this Cheq backup? This replaces your current local Cheq data on this device.");
      ...
      } : { startingNetWorth: 0, createdAt: new Date().toISOString() };
